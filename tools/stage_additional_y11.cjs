...
const IMG_DIR = path.join(__dirname, '..', 'HTML Exams', 'Additional Y11 HTML Exams', 'Extracted Images');
const SLUG = 'additional-y11';
...
(async () => {
  const srcDir = path.join(__dirname, 'additional_y11');
  const files = fs.readdirSync(srcDir).filter(f => /^paper\d+\.json$/.test(f));
  ...
  for (const f of files) {
    ...
    const outName = 'additional-y11-' + f.replace('.json', '') + '.json';
